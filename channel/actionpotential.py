...
def neuron(state, t, params):
    """
    Following the properties of Ekeberg, et al., 1991, ("A computer based
    model for realistic simulations of neural networks") we can create a model of a neuron
...
def find_isi(I_Ext):
    ...
    for i in arange(1,size(t)-1):
        ...
        if ((v2 > vt) & (v0 < v1) & (v2 < v1)):
            peaks = append(peaks, i)
    isi = array([])
    for i in np.arange(size(peaks)-1):
        isi = append(isi, t[peaks[i+1]]-t[peaks[i]])
