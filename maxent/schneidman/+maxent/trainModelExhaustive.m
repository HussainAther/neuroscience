...
while i < num_steps
        ...
        current_dkl = maxent.dkl(empirical_probs,model_probs);
        ...
        if isinf(current_dkl)
            ...
            if ~(current_dkl < previous_dkl - alpha * step_size * norm(factor_gradient)^2)
                ...
                if (use_acceleration)
                    ...
                    test_model = maxent.setFactors(test_model, (maxent.getFactors(test_model) + factor_gradient * step_size));        
                    ...
                    model_probs = exp(model_logprobs);
                    test_dkl = maxent.dkl(empirical_probs,model_probs);
    ...
    if (use_acceleration)
        ...
        y = maxent.getFactors(model) + step_size * factor_gradient;                
        model = maxent.setFactors(model,(1-gamma) .* y + gamma .* yprev);
                

    else
        % regular slow factor_gradient ascent
        model = maxent.setFactors(maxent.getFactors(model) + factor_gradient * step_size);
    end
